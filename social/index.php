...
try {
  ...
  if (!$user->_logged_in) {
    ...
    if (!$system['newsfeed_public']) {
      ...
      // get upgrade options
      if ($system['packages_enabled']) {
        $upgrade_options = $user->get_upgrade_options();
        $smarty->assign('upgrade_options', $upgrade_options);
      }
    ...
    switch ($_GET['view']) {
      ...
      case 'upgrade_posts':
          ...
          // page header -> aqui usuario
          page_header(__("Upgrade Posts"));
  
          // get posts (upgrade)
          $posts = $user->get_posts(array('get' => 'upgrade'));
          /* assign variables */
          $smarty->assign('posts', $posts);
          // var_dump('caiu aqui');
          // die;
      ...
      case 'upgrade_pages':
            ...
            // page header
            page_header(__("Upgrade Pages"));
    
            // get pages (upgrade)
            $upgrade_pages = $user->get_pages(array('upgrade' => true));
            /* assign variables */
            $smarty->assign('upgrade_pages', $upgrade_pages);
            break;
