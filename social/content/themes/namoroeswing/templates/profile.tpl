        ...
        <div class="profile-buttons-wrapper">
          {if $user->_logged_in}
            {if $user->_data['user_id'] != $profile['user_id']}
              ...
              <!-- recommend-->
              {if $profile['i_recommend'] || $this->_data['recommends_ids']}
                <button type="button" class="btn btn-md rounded-pill btn-light js_disrecommend" data-uid="{$profile['user_id']}">
                  <i class="fa fa-thumbs-down mr5"></i>{__("Disrecommend")}
                </button>
              {else}
                <button type="button" class="btn btn-md rounded-pill btn-light js_recommend" data-uid="{$profile['user_id']}">
              ...
              {if $system['packages_enabled'] && !$user->_data['user_subscribed'] || $user->_data['user_package'] == 5 && $user->_data['user_subscribed']}
                 <button type="button" class="btn btn-icon rounded-pill btn-light mlr5" href="{$system['system_url']}/upgrade/posts" data-toggle="modal" data-url="core/upgradepost.php">
                    {include file='__svg_icons.tpl' icon="header-messages" class="main-icon" width="20px" height="20px"}
                 </button>
                {else}
                 <button type="button" class="btn btn-icon rounded-pill btn-light mlr5 js_chat-start" data-uid="{$profile['user_id']}" data-name="{$profile['name']}" data-link="{$profile['user_name']}">
                    {include file='__svg_icons.tpl' icon="header-messages" class="main-icon" width="20px" height="20px"}
                 </button>
              {/if}
                      ...
                      <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                        <div class="modal-dialog modal-sm modal-dialog-rightred">
                          <div class="modal-content">
                            ...
                            <div class="modal-body">                                                    
                              <span class="title"><h5>{__("Recommendations")}</h5></span>  
                            <div class="dropdown-widget-body">
                              <div class="js_scroller">
                                {if $user->_data['recommendations']}
                                  <ul>
                                    {foreach $user->_data['recommendations'] as $recommendation}
         ...
         <div class="container-fluid-md-center">
            <div class="row justify-content-md-center">
              <label align="center">Compartilhar por:
                <br>
                <div class="col-sm-2">
                  <a href=whatsapp://send?text=*%20Olha%20esse%20link%20e%20te%20diverte%20com%20as%20fantasias%20do%20mundo%20liberal!%20*%20{$system['system_url']}/{$profile['user_name']}/>WhatsApp {include file='__svg_icons.tpl' icon="whatsapp" width="24px" height="24px"}</a>
                </div>
                <div class="col-sm-2">
                  <a href=https://telegram.me/share/url?url={$system['system_url']}/{$profile['user_name']}&text=*%20Olha%20esse%20link%20e%20te%20diverte%20com%20as%20fantasias%20do%20mundo%20liberal!%20*%20/>Telegram {include file='__svg_icons.tpl' icon="telegram" width="24px" height="24px"}</a>
                </div>
              </label>
            </div> 
          </div>    
         ...
         <ul class="about-list">
          ...
          <!-- info -->
            ...
            {if $system['location_info_enabled']}
                ...
                {if $profile['user_id'] == $user->_data['user_id'] || $profile['user_privacy_location'] == "public" || ($profile['user_privacy_location'] == "friends" && $profile['we_friends'])}
                  <li>
                    <div class="about-list-item">
                      ...
                      {$ciTyprofile= explode(' - ', $profile['user_current_city'])}
                      {__("Lives in")} <span class="text-dark">{$ciTyprofile[0]} - {$ciTyprofile[1]}</span>
                    </div>
                  </li>
                {/if}
              {/if}
              ...
              <li>
                <div class="about-list-item">
                  <a href=whatsapp://send?text=😈Olha%20esse%20link%20e%20te%20diverte%20com%20as%20fantasias%20do%20mundo%20liberal!😈%20{$system['system_url']}/{$profile['user_name']}/>{include file='__svg_icons.tpl' icon="whatsapp" class="main-icon" width="24px" height="24px"}Compartilhar no WhatsApp</a>
                </div>
              </li>
              <li>
                <div class="about-list-item">
