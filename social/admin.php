...
try {
  ...
  switch ($_GET['view']) {
    ...
    case 'countries':
      ...
      switch ($_GET['sub_view']) {
        case '':
          ...
          // get data
          $get_rows = $db->query("SELECT * FROM system_countries") or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $rows[] = $row;
            }
        ...
        case 'edit':
          ...
          // get data
          $get_data = $db->query(sprintf("SELECT * FROM system_countries WHERE country_id = %s", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          if ($get_data->num_rows == 0) {
            _error(404);
          }
          $data = $get_data->fetch_assoc();
    ...
    case 'genders':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'edit':
          ...
          // get data
          $get_data = $db->query(sprintf("SELECT * FROM system_genders WHERE gender_id = %s", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          if ($get_data->num_rows == 0) {
            _error(404);
          }
          $data = $get_data->fetch_assoc();
    ...
    case 'users':
      ...
      switch ($_GET['sub_view']) {
        case '':
          ...
          $insights = [];
          /* total users */
          $get_users = $db->query("SELECT COUNT(*) as count FROM users" . $cug_where_query) or _error('SQL_ERROR');
          $insights['users'] = $get_users->fetch_assoc()['count'];
          $get_banned = $db->query("SELECT COUNT(*) as count FROM users WHERE user_banned = '1'" . $cug_where_and_query) or _error('SQL_ERROR');
          $insights['banned'] = $get_banned->fetch_assoc()['count'];
          $get_not_activated = $db->query("SELECT COUNT(*) as count FROM users WHERE user_activated = '0'" . $cug_where_and_query) or _error('SQL_ERROR');
          $insights['not_activated'] = $get_not_activated->fetch_assoc()['count'];

          // get data
          require('includes/class-pager.php');
          $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
          $total = $db->query("SELECT COUNT(*) as count FROM users" . $cug_where_query) or _error('SQL_ERROR');
          ...
          $get_rows = $db->query("SELECT * FROM users " . $cug_where_query . "ORDER BY user_id ASC " . $limit_query) or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
        ...
        case 'edit':
          ...
          /* parse birthdate companions*/
          $data['user_birthdate_companions_parsed'] = date_parse($data['user_birthdate_companions']);          
          /* get user sessions */
          ...
          if ($system['monetization_enabled']) {
            $data['can_monetize_content'] = false;
            switch ($system['monetization_permission']) {
              case 'admins':
                if ($data['user_group'] < 3) {
                  $data['can_monetize_content'] = true;
                }
                break;

              case 'pro':
                if ($data['user_group'] < 3 || $data['user_subscribed']) {
                  $data['can_monetize_content'] = true;
                }
                break;

              case 'verified':
                if ($data['user_group'] < 3 || $data['user_subscribed'] || $data['user_verified']) {
                  $data['can_monetize_content'] = true;
                }
                break;
    ...
    case 'users_groups':
      ...
      switch ($_GET['sub_view']) {
        case '':
          ...
          /* get admins count */
          $get_admins = $db->query("SELECT COUNT(*) as count FROM users WHERE user_group = '1'") or _error('SQL_ERROR');
          $counters['admins_count'] = $get_admins->fetch_assoc()['count'];
          /* get moderators count */
          $get_moderators = $db->query("SELECT COUNT(*) as count FROM users WHERE user_group = '2'") or _error('SQL_ERROR');
          $counters['moderators_count'] = $get_moderators->fetch_assoc()['count'];
          /* get users count */
          $get_users = $db->query("SELECT COUNT(*) as count FROM users WHERE user_group = '3' AND user_group_custom = '0'") or _error('SQL_ERROR');
    ...
    case 'posts':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'edit_category':
          ...
          // get data
          $get_data = $db->query(sprintf("SELECT * FROM posts_videos_categories WHERE category_id = %s", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          if ($get_data->num_rows == 0) {
            _error(404);
          }
          $data = $get_data->fetch_assoc();
    ...
    case 'groups':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'edit_group':
          ...
          // get data
          $get_data = $db->query(sprintf("SELECT `groups`.*, users.* FROM `groups` INNER JOIN users ON `groups`.group_admin = users.user_id WHERE `groups`.group_id = %s", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          if ($get_data->num_rows == 0) {
            _error(404);
          }
          $data = $get_data->fetch_assoc();
    ...
    case 'blogs':
      ...
      switch ($_GET['sub_view']) {
          ...
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              ...
              if ($row['user_type'] == "user") {
                ...
                $row['article_author_name'] = ($system['show_usernames_enabled']) ? $row['user_name'] : $row['user_firstname'] . " " . $row['user_lastname'];
              } else {
                /* page */
                $row['article_author_picture'] = get_picture($row['page_picture'], "page");
                $row['article_author_url'] = $system['system_url'] . '/pages/' . $row['page_name'];
                $row['article_author_name'] = $row['page_title'];
              }
              $rows[] = $row;
            }
          }
        ...
        case 'find':
          ...
          require('includes/class-pager.php');
          $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
          $total = $db->query(sprintf('SELECT COUNT(*) as count FROM posts INNER JOIN posts_articles ON posts.post_id = posts_articles.post_id AND posts.post_type = "article" LEFT JOIN users ON posts.user_id = users.user_id AND posts.user_type = "user" LEFT JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = "page" WHERE NOT (users.user_name <=> NULL AND pages.page_name <=> NULL) AND posts.text LIKE %1$s OR posts_articles.title LIKE %1$s OR posts_articles.text LIKE %1$s OR posts_articles.tags LIKE %1$s', secure($_GET['query'], 'search'))) or _error('SQL_ERROR');
          $params['total_items'] = $total->fetch_assoc()['count'];
          $params['items_per_page'] = $system['max_results'];
          $params['url'] = $system['system_url'] . '/' . $control_panel['url'] . '/blogs/find?query=' . $_GET['query'] . '&page=%s';
          $pager = new Pager($params);
          $limit_query = $pager->getLimitSql();
          $get_rows = $db->query(sprintf('SELECT posts.*, posts_articles.title, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, pages.* FROM posts INNER JOIN posts_articles ON posts.post_id = posts_articles.post_id AND posts.post_type = "article" LEFT JOIN users ON posts.user_id = users.user_id AND posts.user_type = "user" LEFT JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = "page" WHERE NOT (users.user_name <=> NULL AND pages.page_name <=> NULL) AND posts.text LIKE %1$s OR posts_articles.title LIKE %1$s OR posts_articles.text LIKE %1$s OR posts_articles.tags LIKE %1$s ' . $limit_query, secure($_GET['query'], 'search'))) or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $row['article_title_url'] = get_url_text(html_entity_decode($row['title'], ENT_QUOTES));
              /* check the post author type */
    ...
    case 'market':
      ...
      switch ($_GET['sub_view']) {
        case '':
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Market"));
          ...
          $params['total_items'] = $total->fetch_assoc()['count'];
          $params['items_per_page'] = $system['max_results'];
          $params['url'] = $system['system_url'] . '/' . $control_panel['url'] . '/market?page=%s';
          $pager = new Pager($params);
          $limit_query = $pager->getLimitSql();
        ...
        case 'find':
          ...
          require('includes/class-pager.php');
          $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
          $total = $db->query(sprintf('SELECT COUNT(*) as count FROM posts INNER JOIN posts_products ON posts.post_id = posts_products.post_id AND posts.post_type = "product" LEFT JOIN users ON posts.user_id = users.user_id AND posts.user_type = "user" LEFT JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = "page" WHERE NOT (users.user_name <=> NULL AND pages.page_name <=> NULL) AND posts.text LIKE %1$s OR posts_products.name LIKE %1$s OR posts_products.location LIKE %1$s', secure($_GET['query'], 'search'))) or _error('SQL_ERROR');
          $params['total_items'] = $total->fetch_assoc()['count'];
          $params['items_per_page'] = $system['max_results'];
          $params['url'] = $system['system_url'] . '/' . $control_panel['url'] . '/market/find?query=' . $_GET['query'] . '&page=%s';
          $pager = new Pager($params);
          $limit_query = $pager->getLimitSql();
          $get_rows = $db->query(sprintf('SELECT posts.*, posts_products.name, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, pages.* FROM posts INNER JOIN posts_products ON posts.post_id = posts_products.post_id AND posts.post_type = "product" LEFT JOIN users ON posts.user_id = users.user_id AND posts.user_type = "user" LEFT JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = "page" WHERE NOT (users.user_name <=> NULL AND pages.page_name <=> NULL) AND posts.text LIKE %1$s OR posts_products.name LIKE %1$s OR posts_products.location LIKE %1$s ' . $limit_query, secure($_GET['query'], 'search'))) or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              /* check the post author type */
              if ($row['user_type'] == "user") {
    ...
    case 'games':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'edit_genre':
          ...
          // get data
          $get_data = $db->query(sprintf("SELECT * FROM games_genres WHERE genre_id = %s", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          if ($get_data->num_rows == 0) {
            _error(404);
          }
          $data = $get_data->fetch_assoc();
    ...
    case 'ads':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'users_ads':
          ...
          // get data
          $get_rows = $db->query("SELECT ads_campaigns.*, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture FROM ads_campaigns INNER JOIN users ON ads_campaigns.campaign_user_id = users.user_id WHERE ads_campaigns.campaign_is_declined = '0'") or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $row['user_picture'] = get_picture($row['user_picture'], $row['user_gender']);
    ...
    case 'wallet':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'payments':
          ...
          // get data
          $get_rows = $db->query("SELECT wallet_payments.*, users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture FROM wallet_payments INNER JOIN users ON wallet_payments.user_id = users.user_id WHERE wallet_payments.status = '0'") or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $row['user_picture'] = get_picture($row['user_picture'], $row['user_gender']);
              switch ($row['method']) {
    ...
    case 'pro':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'packages':
          ...
          // get data
          $get_rows = $db->query("SELECT packages.*, permissions_groups.permissions_group_title FROM packages LEFT JOIN permissions_groups ON packages.package_permissions_group_id = permissions_groups.permissions_group_id") or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
        ...
        case 'edit':
          ...
          // get data
          $get_data = $db->query(sprintf("SELECT packages.*, permissions_groups.permissions_group_title FROM packages LEFT JOIN permissions_groups ON packages.package_permissions_group_id = permissions_groups.permissions_group_id WHERE packages.package_id = %s", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          if ($get_data->num_rows == 0) {
            _error(404);
          }
          $data = $get_data->fetch_assoc();
        ...
        case 'earnings':
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Pro System") . " &rsaquo; " . __("Earnings"));
    ...
    case 'funding':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'payments':
          ...
          // get data
          $get_rows = $db->query("SELECT funding_payments.*, users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture FROM funding_payments INNER JOIN users ON funding_payments.user_id = users.user_id WHERE funding_payments.status = '0'") or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $row['user_picture'] = get_picture($row['user_picture'], $row['user_gender']);
              switch ($row['method']) {
        ...
        case 'requests':
          ...
          require('includes/class-pager.php');
          $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
          $total = $db->query("SELECT COUNT(*) as count FROM posts INNER JOIN posts_funding ON posts.post_id = posts_funding.post_id AND posts.post_type = 'funding' INNER JOIN users ON posts.user_id = users.user_id AND posts.user_type = 'user'") or _error('SQL_ERROR');
          $params['total_items'] = $total->fetch_assoc()['count'];
          $params['items_per_page'] = $system['max_results'];
    ...
    case 'monetization':
      ...
      switch ($_GET['sub_view']) {
        case '':
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Content Monetization"));
          break;

        case 'payments':
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Content Monetization") . " &rsaquo; " . __("Payment Requests"));

          // get data
          $get_rows = $db->query("SELECT monetization_payments.*, users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture FROM monetization_payments INNER JOIN users ON monetization_payments.user_id = users.user_id WHERE monetization_payments.status = '0'") or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $row['user_picture'] = get_picture($row['user_picture'], $row['user_gender']);
              switch ($row['method']) {
    ...
    case 'reports':
      ...
      switch ($_GET['sub_view']) {
        case '':
          ...
          // get data
          $get_rows = $db->query("SELECT reports.*, reports_categories.category_name, users.user_name, users.user_firstname, users.user_lastname, users.user_picture, users.user_gender FROM reports INNER JOIN users ON reports.user_id = users.user_id LEFT JOIN reports_categories ON reports.category_id = reports_categories.category_id") or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $row['user_picture'] = get_picture($row['user_picture'], $row['user_gender']);
              /* get reported node */
        ...
        case 'edit_category':
          ...
          // get data
          $get_data = $db->query(sprintf("SELECT * FROM reports_categories WHERE category_id = %s", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          if ($get_data->num_rows == 0) {
            _error(404);
          }
          $data = $get_data->fetch_assoc();
    ...
    case 'tools':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'auto-connect':
          ...
          $get_rows = $db->query("SELECT * FROM auto_connect") or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $rows[$row['type']][] = $row;
            }
    ...
    case 'reactions':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'edit':
          ...
          // get data
          $get_data = $db->query(sprintf("SELECT * FROM system_reactions WHERE reaction_id = %s", secure($_GET['id'], 'int'))) or _error('SQL_ERROR');
          if ($get_data->num_rows == 0) {
            _error(404);
          }
          $data = $get_data->fetch_assoc();
  ...
  if ($user->_is_admin) {
    ...
    $get_wallet_payments = $db->query("SELECT COUNT(*) as count FROM wallet_payments INNER JOIN users ON wallet_payments.user_id = users.user_id WHERE wallet_payments.status = '0'") or _error('SQL_ERROR');
    ...
    $get_affiliates_payments = $db->query("SELECT COUNT(*) as count FROM affiliates_payments INNER JOIN users ON affiliates_payments.user_id = users.user_id WHERE affiliates_payments.status = '0'") or _error('SQL_ERROR');
    ...
    $get_points_payments = $db->query("SELECT COUNT(*) as count FROM points_payments INNER JOIN users ON points_payments.user_id = users.user_id WHERE points_payments.status = '0'") or _error('SQL_ERROR');
    ...
    $get_funding_payments = $db->query("SELECT COUNT(*) as count FROM funding_payments INNER JOIN users ON funding_payments.user_id = users.user_id WHERE funding_payments.status = '0'") or _error('SQL_ERROR');
    ...
    $get_monetization_payments = $db->query("SELECT COUNT(*) as count FROM monetization_payments INNER JOIN users ON monetization_payments.user_id = users.user_id WHERE monetization_payments.status = '0'") or _error('SQL_ERROR');
    ...
    $get_bank_transfers = $db->query("SELECT COUNT(*) as count FROM bank_transfers INNER JOIN users ON bank_transfers.user_id = users.user_id LEFT JOIN packages ON bank_transfers.package_id = packages.package_id WHERE bank_transfers.status = '0'") or _error('SQL_ERROR');
  ...
  /* reports insights */
  $get_reports = $db->query("SELECT COUNT(*) as count FROM reports INNER JOIN users ON reports.user_id = users.user_id") or _error('SQL_ERROR');
