...
try {
  ...
  switch ($_GET['view']) {
    ...
    case 'profile':
      ...
      switch ($_GET['sub_view']) {
        case '':
          ...
          $user->_data['user_birthdate_companions_parsed'] = ($user->_data['user_birthdate_companions']) ? date_parse($user->_data['user_birthdate_companions']) : null;
    ...
    case 'accounts':
      // page header
      page_header(__("Settings") . " &rsaquo; " . __("Accounts"));
      break;
    ...
    case 'linked':
      ...
      if (!$system['facebook_login_enabled'] && !$system['twitter_login_enabled'] && !$system['google_login_enabled'] && !$system['linkedin_login_enabled'] && !$system['vkontakte_login_enabled'] && !$system['wordpress_login_enabled']) {
        _error(404);
      }
    ...
    case 'monetization':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'payments':
          ...
          // get payments
          $payments = [];
          $get_payments = $db->query(sprintf('SELECT * FROM monetization_payments WHERE user_id = %s', secure($user->_data['user_id'], 'int'))) or _error('SQL_ERROR');
    ...
    case 'funding':
      ...
      // get payments
      $payments = [];
      $get_payments = $db->query(sprintf('SELECT * FROM funding_payments WHERE user_id = %s', secure($user->_data['user_id'], 'int'))) or _error('SQL_ERROR');
      if ($get_payments->num_rows > 0) {
        while ($payment = $get_payments->fetch_assoc()) {
          $payments[] = $payment;
        }
