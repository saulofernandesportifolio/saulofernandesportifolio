  ...
  switch ($_GET['do']) {
    case 'edit_account':
      ...
      if (strtolower($_POST['user_name']) != strtolower($user_info['user_name'])) {
        ...
        if ($user->reserved_username($_POST['user_name'])) {
      ...
      /* check if demo super admin user */
      if ($_POST['user_demo'] && $_GET['id'] == '1') {
        modal("MESSAGE", __("System Message"), __("You can't demo this user"));
      }
      /* check user group */
      if (substr($_POST['user_group'], 0, 5) == "cusg_") {
        $user_group = '3';
        $custom_user_group = substr($_POST['user_group'], 5);
        /* check if this custom user group is valid */
        if (!$user->check_user_group($custom_user_group)) {
          throw new Exception(__("Please select a valid user group"));
        }
      } else {
        $user_group = (!in_array($_POST['user_group'], ['1', '2', '3'])) ? '3' : $_POST['user_group'];
        $custom_user_group = 0;
      }
      /* update */
      $db->query(sprintf("UPDATE users SET user_verified = %s, user_banned = %s, user_banned_message = %s, user_activated = %s, user_demo = %s, user_group = %s, user_group_custom = %s, user_name = %s, user_email = %s, user_email_verified = %s, user_phone = %s, user_phone_verified = %s, user_password = %s WHERE user_id = %s", secure($_POST['user_verified']), secure($_POST['user_banned']), secure($_POST['user_banned_message']), secure($_POST['user_activated']), secure($_POST['user_demo']), secure($user_group, 'int'), secure($custom_user_group, 'int'), secure($_POST['user_name']), secure($_POST['user_email']), secure($_POST['user_email_verified']), ($_POST['user_phone'] == null) ? 'null' : secure($_POST['user_phone']), secure($_POST['user_phone_verified']), secure($_POST['user_password']), secure($_GET['id'], 'int'))) or _error('SQL_ERROR_THROWEN');
      /* return */
    ...
    case 'edit_profile':
      ...
      if ($_POST['birth_year_companions'] != "none" && $_POST['birth_month_companions'] != "none" && $_POST['birth_day_companions'] != "none") {
         $_POST['user_birthdate_companions'] = $_POST['birth_year_companions'] . '-' . $_POST['birth_month_companions'] . '-' . $_POST['birth_day_companions'];
      } else {
      ...
      $db->query(sprintf("UPDATE users 
                          SET user_firstname = %s, 
                              ...
                              user_edu_school = %s, 
                              user_edu_class = %s, 
                              user_social_facebook = %s, 
                              user_social_twitter = %s, 
                              user_social_youtube = %s, 
                              user_social_instagram = %s, 
                              user_social_linkedin = %s, 
                              user_social_vkontakte = %s 
                              WHERE user_id = %s", 
                              secure($_POST['user_firstname']), 
                              secure($_POST['user_lastname']), 
                              secure($_POST['user_country'], 'int'), 
                              ...
                              secure($_POST['user_work_place']), 
                              secure($_POST['user_work_url']), 
                              secure($_POST['user_current_city']), 
                              secure($_POST['user_hometown']), 
                              secure($_POST['user_edu_major']), 
                              secure($_POST['user_edu_school']), 
                              secure($_POST['user_edu_class']), 
                              secure($_POST['facebook']), 
                              secure($_POST['twitter']), 
                              secure($_POST['youtube']), 
                              secure($_POST['instagram']), 
                              secure($_POST['linkedin']), 
                              secure($_POST['vkontakte']), 
    ...
    case 'edit_privacy':
      ...
      if (!in_array($_POST['user_privacy_poke'], $privacy) || !in_array($_POST['user_privacy_gifts'], $privacy) || !in_array($_POST['user_privacy_wall'], $privacy) || !in_array($_POST['user_privacy_gender'], $privacy) || !in_array($_POST['user_privacy_relationship'], $privacy) || !in_array($_POST['user_privacy_birthdate'], $privacy) || !in_array($args['user_privacy_birthdate_companions'], $privacy) || !in_array($_POST['user_privacy_basic'], $privacy) || !in_array($_POST['user_privacy_work'], $privacy) || !in_array($_POST['user_privacy_location'], $privacy) || !in_array($_POST['user_privacy_education'], $privacy) || !in_array($_POST['user_privacy_other'], $privacy) || !in_array($_POST['user_privacy_friends'], $privacy) || !in_array($_POST['user_privacy_followers'], $privacy) || !in_array($_POST['user_privacy_photos'], $privacy) || !in_array($_POST['user_privacy_pages'], $privacy) || !in_array($_POST['user_privacy_groups'], $privacy) || !in_array($_POST['user_privacy_events'], $privacy)) {
        _error(400);
      }
      /* update user */
      $db->query(sprintf(
        ...
        secure($_POST['user_privacy_birthdate_companions']),
        ...
        secure($_POST['user_privacy_followers']),
    ...
    case 'edit_extra':
      ...
      /* monetization */
      $_POST['user_monetization_enabled'] = (isset($_POST['user_monetization_enabled'])) ? '1' : '0';
      /* check monetization plans */
      $monetization_plans_count = $user->get_monetization_plans_count($_GET['id']);
      if ($_POST['user_monetization_enabled'] && $monetization_plans_count == 0) {
        throw new Exception(__("You must add monetization plans first"));
      }
      /* update */
      $db->query(sprintf("UPDATE users SET user_wallet_balance = %s, user_points = %s, user_monetization_enabled = %s WHERE user_id = %s", secure($_POST['user_wallet_balance']), secure($_POST['user_points']), secure($_POST['user_monetization_enabled']), secure($_GET['id'], 'int'))) or _error('SQL_ERROR_THROWEN');
      /* return */
      return_json(array('success' => true, 'message' => __("User info have been updated")));
      break;
