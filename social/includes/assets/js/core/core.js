...
// modal
function modal() {
  if (arguments[0] == "#modal-login" || arguments[0] == "#chat-calling" || arguments[0] == "#chat-ringing" || arguments[0] == 'modal-upgrade') {
...
$(function () {
    ...
    $("#gender").change(function(){
        $(this).find("option:selected").each(function(){
            ...
            if(optionValue == 3 || optionValue == 4 || optionValue == 5){
                $("#birth_companions").show();
            } else 
            if(optionValue != 3 || optionValue != 4 || optionValue != 5){
              $("#birth_companions").hide();
              $('#month_companions').val('none');
              $('#day_companions').val('none');
              $('#year_companions').val('none');
              $('#birth_month_companions').val('none');
              $('#birth_day_companions').val('none');
              $('#birth_year_companions').val('none');
           }
        });
    }).change();
  });
  ...
  $('#gender').on('click', function (event) {
    event.preventDefault;
    //alert('caiu aqui');
    $('#user_relationship').val('none');
    return false;
  });


// botões restriction idade
  $("#age_restriction").click(function() { 

    var botao = document.getElementById('age_restriction_companions');
    botao.click();
  });
...
// inicio api ibge  estados e cidades start sistema
  let url ='https://servicodados.ibge.gov.br/api/v1/localidades/estados?orderBy=nome';
   ...
   $.getJSON(url, function(data){
      ...
      $.each(data, function(v, val){
         conteudo += '<option value='+val.id+'>'+val.sigla+'</option>'; 
      });
      $('#listaState').html(conteudo); 
  });

  $(document).ready(function(){

    $("#listaState").change(function(){   
      ...
      $(this).find("option:selected").each(function(){      
        ...
        let urlcity= 'https://servicodados.ibge.gov.br/api/v1/localidades/estados/'+optionValue+'/distritos?orderBy=nome';
        ...
        $.getJSON(urlcity, function(dataCity){
            ...
            let conteudoCity = '';
            conteudoCity += '<option value="">Selecione</option>';             
            for (let valCity in  dataCity){
              //exibe as propriedades do objeto 
              //console.log(dataCity[valCity]);
              conteudoCity += '<option value="'+dataCity[valCity].nome+' - '+dataCity[valCity].municipio.microrregiao.mesorregiao.UF.sigla+' - '+dataCity[valCity].municipio.microrregiao.mesorregiao.UF.id+'">'+dataCity[valCity].nome+'</option>';  
            }

           $('#city').html(conteudoCity);
        });
      });
    }).change();  
  });
  // fim api ibge  estados e cidades em start sistema



 // inicio api ibge  estados e cidades profile 
  $(document).ready(function(){

    $("#listaState2").change(function(){   
      ...
      $(this).find("option:selected").each(function(){      
        ...
        // inicio api ibge  estados e cidades
        let urlProfile ='https://servicodados.ibge.gov.br/api/v1/localidades/estados?orderBy=nome';
        let urlProfile2= 'https://servicodados.ibge.gov.br/api/v1/localidades/estados/'+optionValue+'/distritos?orderBy=nome';
        

        //console.log(urlProfile2);
        $ufProfile = '';

        //registron JSON
        $.getJSON(urlProfile2, function(dataProfile2){
          for (var i = 0; i < 1; i++) {
            console.log(dataProfile2[i].municipio.microrregiao.mesorregiao.UF.sigla);
            $ufProfile += dataProfile2[i].municipio.microrregiao.mesorregiao.UF.sigla;
          
          }
        });

        //registron JSON
        $.getJSON(urlProfile, function(dataProfile){

            let conteudoProfile = '';
            conteudoProfile += '<option value="'+optionValue+'">'+$ufProfile+'</option>';  
          $.each(dataProfile, function(v, val){
            conteudoProfile += '<option value='+val.id+'>'+val.sigla+'</option>'; 
          });
          $('#listaState2').html(conteudoProfile); 
        });


        //registron JSON
        $.getJSON(urlProfile2, function(dataCity22){

          let conteudoCity22 = '';
          //conteudoCity2 += '<option value="'+$ciTyteste+'">'+$ciTyteste[0]+'</option>';
          conteudoCity22 += '<option value="">Selecione</option>';               
          for (let valCity22 in  dataCity22){
            //exibe as propriedades do objeto 
            //console.log(dataCity[valCity]);
            conteudoCity22 += '<option value="'+dataCity22[valCity22].nome+' - '+dataCity22[valCity22].municipio.microrregiao.mesorregiao.UF.sigla+' - '+dataCity22[valCity22].municipio.microrregiao.mesorregiao.UF.id+'">'+dataCity22[valCity22].nome+'</option>';  
          }

          $('#city2').html(conteudoCity22);
        }); 
  ...
  $(document).ready(function(){
      ...
      $("#city2").change(function(){   
        ...
        $(this).find("option:selected").each(function(){   
          ...
          var ciTyteste = optionValue.split(' - ');

          console.log(ciTyteste);   
          
          let urlProfileCity2= 'https://servicodados.ibge.gov.br/api/v1/localidades/estados/'+ciTyteste[2]+'/distritos?orderBy=nome';

          //registron JSON
          $.getJSON(urlProfileCity2, function(dataCity2){

            $("#hometown").val(ciTyteste[0]+' - '+ciTyteste[1]+' - '+ciTyteste[2]);

            let conteudoCity2 = '';
            
            conteudoCity2 += '<option selected value="'+ciTyteste[0]+' - '+ciTyteste[1]+' - '+ciTyteste[2]+'">'+ciTyteste[0]+'</option>';             
            for (let valCity2 in  dataCity2){
              //exibe as propriedades do objeto 
              //console.log(dataCity[valCity]);
              conteudoCity2 += '<option value="'+dataCity2[valCity2].nome+' - '+dataCity2[valCity2].municipio.microrregiao.mesorregiao.UF.sigla+' - '+dataCity2[valCity2].municipio.microrregiao.mesorregiao.UF.id+'">'+dataCity2[valCity2].nome+'</option>';  
            }

            $('#city2').html(conteudoCity2);
          });        

        });
      }).change(); 
  });
  //fim api ibge  estados e cidades profile
  ...
  $(document).ready(function () {
    ...
    //var valueElement_a = document.getElementById('age_a').getAttribute('value');
    var valueElement_a = $('#age_a').attr('value');
      // console.log(valueElement_a);
      // console.log(getAge(valueElement_a)) // 31
      $('#age1').html(getAge(valueElement_a));
  ...
  $(document).ready(function () {
    ...
    //var valueElement_b = document.getElementById('age_b').getAttribute('value');
    var valueElement_b = $('#age_b').attr('value');
      // console.log(valueElement_b);
      // console.log(getAge(valueElement_b)) // 31
      $('#age2').html(getAge(valueElement_b));

  });

  $(document).ready(function () {

    //alert($('textarea').text().trim());
