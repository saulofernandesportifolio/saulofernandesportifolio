...
$(function () {
  ...
  $('body').on('click', '.js_publisher', function () {
    ...
    var video_category = attachments_video.find('select').val();
    /* get audio */
  ...
  $('body').on('click', '.js_voice-start', function () {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then(function (stream) {
      ...
      voice_recording_object.onComplete = function (recorder, blob) {
        ...
        formData.append('file', blob, guid() + "." + voice_recording_encoding);
  ...
  $('body').on('click', '.js_hide-author', function () {
    ...
    var author_name = $(this).data('author-name');
    var id = post.data('id');
    $.post(api['users/connect'], { 'do': 'disrecommend', 'id': author_id }, function (response) {
      if (response.callback) {
        eval(response.callback);
      } else {
        post.hide();
  ...
  $('body').on('click', '.js_unhide-author', function () {
    ...
    var author_name = $(this).data('author-name');
    var id = post.data('id');
    $.post(api['users/connect'], { 'do': 'recommend', 'id': author_id }, function (response) {
      if (response.callback) {
        eval(response.callback);
      } else {
        post.prev().show();
