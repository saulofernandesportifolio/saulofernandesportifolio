...
function noty_notification(image, message, url) {
  let toast = $(render_template('#toast-notification', { 'image': image, 'message': message }));
...
function change_profile_pciture(image_path) {
    ...
    if ($(this)[0].nodeName != "TAGS") {
      $(this).tagify({
        ...
        .on("input", function (e, tagName) {
          ...
          $.post(api['users/tagify'], { 'query': tagName.value, 'handle': $(this).data('handle') }, function (response) {
            /* check the response */
            if (response.callback) {
              eval(response.callback);
            } else {
...
$(function () {
  ...
  if ($(".js_wysiwyg").length > 0) {
    if (!tinymce_photos_enabled) {
      tinymce.init({
        ...
        language_url: 'https://cdn.jsdelivr.net/npm/tinymce-i18n@22.5.2/langs6/' + system_langauge_code + '.js',
        branding: false,
        height: 300,
        relative_urls: false,
        convert_urls: true,
  ...
  if ($(".js_wysiwyg-advanced").length > 0) {
    tinymce.init({
      ...
      plugins:
        ...
        'save table directionality'
      ,
      extended_valid_elements: "script[src|async|defer|type|charset],marquee[class|width|height|align|onmouseover|onmouseout|behavior|direction|scrollamount|scrolldelay]",
      image_advtab: true,
      paste_data_images: true,
      images_upload_handler: (blobInfo, progress) => new Promise((resolve, reject) => {
        var xhr, formData;
  ...
  $('body').on('click', '.js_tag-add', function () {
    ...
    var tag = '<li data-uid="' + uid + '">' + name + '<button type="button" class="btn-close js_tag-remove" title="' + __['Remove'] + '"></button></li>'
      ...
      if (response.callback) {
        ...
        if (type == "photos") {
          ...
          if (handle == "cover-user" || handle == "cover-page" || handle == "cover-group" || handle == "cover-event") {
            ...
            var image_path = uploads_path + '/' + response.file;
            change_profile_pciture(image_path);
  ...
  $('body').on('click', '.js_profile-picture-change', function () {
    var id = $(this).data('id');
    var handle = $(this).data('type');
    var image = $(this).data('image');
    /* init loading modal */
    modal('#modal-loading', {}, 'default');
    $.post(api['users/image_change'], { 'id': id, 'handle': handle, 'image': image }, function (response) {
      /* check the response */
      if (response.callback) {
        eval(response.callback);
      } else {
  ...
  /* Disrecommend & Recommend */
  $('body').on('click', '.js_recommend, .js_disrecommend', function () {
    var _this = $(this);
    var id = _this.data('uid');
    var _do = (_this.hasClass('js_recommend')) ? 'recommend' : 'disrecommend';
    /* button loading */
    button_status(_this, "loading");
    /* post the request */
    $.post(api['users/connect'], { 'do': _do, 'id': id }, function (response) {
      ...
      if (response.callback) {
        ...
        if (_do == 'recommend') {
          _this.removeClass('js_recommend').addClass('js_disrecommend');
          _this.html('<i class="fa fa-thumbs-down mr5"></i>' + __['Disrecommend']);
        } else {
          _this.removeClass('js_disrecommend').addClass('js_recommend');
          _this.html('<i class="fa fa-thumbs-up mr5"></i>' + __['Recommend']);
        }
      }
    }, "json")
      .fail(function () {
