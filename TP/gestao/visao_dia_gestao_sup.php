...
set_time_limit($tempo);
  ...
  include("../../tp/conexao.php");
  $atv_op2="SELECT * FROM base_gestao WHERE data_tramite ='$dt_dia'";
...
?>
           <p id="p_padrao" align="center">Visão Gestão Dia: <?php echo $dt_dia2 ?></p>
            <hr width="104%">       
             <?php
			 //tabela por regional - inicio
			 
						include("../../tp/conexao.php");
				$atv_op="SELECT * FROM base_gestao WHERE data_tramite ='$dt_dia'";
               ...
               $atv_op="SELECT COUNT(pedido)as pedido,usuario,disc_status_tp,regional FROM base_gestao WHERE data_tramite = '$dt_dia' GROUP BY regional";
                        $acao_op=mysql_query($atv_op,$conecta);
		...
		while ($dado= mysql_fetch_array($acao_op))
				...
				$atv_op="SELECT * FROM base_gestao WHERE data_tramite ='$dt_dia' ";
					...
					$dado= mysql_fetch_array($acao_op);
                        ...
                        $atv_op="SELECT COUNT(pedido)as pedido,usuario,disc_status_tp,status FROM base_gestao WHERE data_tramite = '$dt_dia' GROUP BY status";
                        $acao_op=mysql_query($atv_op,$conecta);
			  ...
			  //tabela por motivo - inicio
				...
				$atv_op="SELECT * FROM base_gestao WHERE data_tramite ='$dt_dia' ";
					...
					$dado= mysql_fetch_array($acao_op);
                        ...
                        $atv_op="SELECT COUNT(pedido)as pedido,termino_efetivo,usuario,disc_status_tp,status_do_cliente FROM base_gestao WHERE data_tramite = '$dt_dia' GROUP BY status_do_cliente";
                        $acao_op=mysql_query($atv_op,$conecta);
						          
		echo "<table bgcolor='D6CA98' align='reight' border='0'>
		<tr>
		<td id=\"t_td\" width=\"15%\">
        <strong><font color=\"#000000\">Status do cliente</font></strong>
        </td>
           ...
           <?php
			//tabela por operador - inicio
					...
					$dado= mysql_fetch_array($acao_op);
                        ...
                        $atv_op="SELECT COUNT(pedido)as pedido ,regional,usuario,disc_status_tp,termino_efetivo,tramite,nome2,data_tramite,turno FROM base_gestao WHERE data_tramite = '$dt_dia' and (tramite = 'Aberto' or tramite = 'Tratando' or tramite = 'Tratado' or tramite = 'Aguardando') GROUP BY usuario,disc_status_tp  DESC";
                        $acao_op=mysql_query($atv_op,$conecta);
                        $num2=mysql_num_rows($acao_op);
		//echo $atv_op;				          
