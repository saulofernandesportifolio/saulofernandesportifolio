...
{
echo "<script>alert('Nenhum regional selecionado.'); document.location.replace('filtro_retornar_para_visao_gestao.php'); </script>\n";
exit;
}
if (empty($_POST["tramite"]))
{
echo "<script>alert('Nenhuma tramite selecionada.'); document.location.replace('filtro_retornar_para_visao_gestao.php'); </script>\n";
exit;
}
if (empty($_POST["turno2"]))
{
echo "<script>alert('Nenhuma turno selecionada.'); document.location.replace('filtro_retornar_para_visao_gestao.php'); </script>\n";
exit;
}	
...
if($data_1 <> ''){
$sql_consulta="SELECT * FROM base_gestao WHERE ($regional) and ($tramite) and ($turno) and termino_efetivo BETWEEN '$data_1' and '$data_2' ORDER BY termino_efetivo";
$sql=$sql_consulta; 
...
if($numero_pedido <> 0){
$sql_consulta="SELECT * FROM base_gestao WHERE pedido='$numero_pedido' ORDER BY termino_efetivo";
$sql=$sql_consulta; 
$acao = mysql_query($sql) or die (mysql_error());
$num = mysql_num_rows($acao);
}
...
if($operador <> '0'){
$sql_consulta="SELECT * FROM base_gestao WHERE ($regional) and ($tramite) and ($turno) and termino_efetivo BETWEEN '$data_1' and '$data_2' and usuario = '$operador' ORDER BY termino_efetivo";
$sql=$sql_consulta; 
$acao = mysql_query($sql) or die (mysql_error());
$num = mysql_num_rows($acao);
}
...
?>
    ...
    <form name="form1" method="post" action="retornar_para_visao_gestao2.php">
...
//Pesquisa e retorna os campos declarado nas variáveis.
		...
		while ($dado=mysql_fetch_array($acao))
			{
              ...
              <td> 
             <input name="ling[]" type="checkbox" id="ling[]" value="<?php echo "$id_gestao" ?>">
              </td>
 ...
 <input type="button" name="Submit2" value="Voltar" onClick=<?php 
		...
		$turno2 ="0";
		?>
		"window.location='filtro_retornar_para_visao_gestao.php'"/><input type="submit" name="Submit" value="Avançar" />
          </td>
          </tr>
