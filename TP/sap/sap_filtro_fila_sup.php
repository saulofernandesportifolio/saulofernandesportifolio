...
{
echo "<script>alert('Nenhum erro selecionado.'); document.location.replace('sap_filtro_sup.php'); </script>\n";
exit;
}
if (empty($_POST["regional"]))
{
echo "<script>alert('Nenhuma regional selecionada.'); document.location.replace('sap_filtro_sup.php'); </script>\n";
exit;
}
...
$sql_consulta="Select * from diario_sap_bko where (fila = 2 or fila = 1 ) and ($novavar2) and ($novavar) ORDER BY data_cadastro ASC";
...
echo "<script type=\"text/javascript\">
        document.location.replace('../../tp/sap/pedido_sap_bko_sup.php');
		</script>";
        exit;
