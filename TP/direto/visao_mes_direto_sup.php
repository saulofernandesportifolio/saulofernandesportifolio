...
set_time_limit($tempo);
 ...
 $dt_dia = date("Y-m-");
  ...
  //echo $dt_dia ;
   ...
   include("../../tp/conexao.php");
  $atv_op2="SELECT * FROM ilha_reversao_direto_bko WHERE data_tramite like '%$dt_dia%'";
...
?>                     
         ...
         <p id="p_padrao" align="center">Visão direto Geral Mês : <?php echo $dt_mes ?></p>
        <hr width="104%">          
             <?php
			 //tabela por regional - inicio
			 
					include("../../tp/conexao.php");
				$atv_op="SELECT * FROM ilha_reversao_direto_bko WHERE data_tramite like '%$dt_dia%'";
                        ...
                        $atv_op="SELECT COUNT(pedido)as pedido,data_tramite,usuario,disc_status_tp,regional FROM ilha_reversao_direto_bko WHERE data_tramite like '%$dt_dia%' GROUP BY regional";
                        $acao_op=mysql_query($atv_op,$conecta);
		...
		while ($dado= mysql_fetch_array($acao_op))
				...
				$atv_op="SELECT * FROM ilha_reversao_direto_bko WHERE data_tramite like '%$dt_dia%' ";
					...
					$dado= mysql_fetch_array($acao_op);
                        ...
                        $atv_op="SELECT COUNT(pedido)as pedido,data_tramite,usuario,disc_status_tp,tipo_erro FROM ilha_reversao_direto_bko WHERE data_tramite like '%$dt_dia%' GROUP BY tipo_erro";
                        $acao_op=mysql_query($atv_op,$conecta);
			  ...
			  //tabela por motivo - inicio
				...
				$atv_op="SELECT * FROM ilha_reversao_direto_bko WHERE data_tramite like '%$dt_dia%' ";
					...
					$dado= mysql_fetch_array($acao_op);
                        ...
                        $atv_op="SELECT COUNT(pedido)as pedido,data_tramite,usuario,disc_status_tp,descricao_erro FROM ilha_reversao_direto_bko WHERE data_tramite like '%$dt_dia%' GROUP BY descricao_erro";
                        $acao_op=mysql_query($atv_op,$conecta);
...
//Pesquisa e retorna os campos declarado nas variáveis.
		...
		while ($dado= mysql_fetch_array($acao_op))
			...
			//tabela por operador - inicio
                        ...
                        $atv_op="SELECT COUNT(pedido)as pedido, regional,usuario,disc_status_tp,criado_em,tramite,nome2,data_tramite,turno FROM ilha_reversao_direto_bko WHERE data_tramite like '%$dt_dia%' and tramite = 'Tratando' or tramite = 'Tratado' or tramite = 'Aguardando' GROUP BY  tramite, usuario DESC";
                        $acao_op=mysql_query($atv_op,$conecta);
...
<a href="javascript:abrir('pedidos_direto_detalhes_sup.php?&login_usuario=<?php echo $usuario ?>&tramite=<?php echo $tramite ?>&turnoo=<?php echo $turnoo ?>&pesquisa_data=<?php echo $pesquisa_data ?>');"><?php echo $pedido ?></a>
