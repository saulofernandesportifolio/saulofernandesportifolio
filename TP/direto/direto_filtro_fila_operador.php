...
{
echo "<script>alert('Nenhuma regional selecionada.'); document.location.replace('../../tp/direto/direto_filtro_operador.php'); </script>\n";
exit;
}
//Regional
$nome = $_SESSION["nome"];
...
$sql_consulta="SELECT * from ilha_reversao_direto_bko WHERE (fila = 2) and (usuario = '$login2')and ($novavar2) and ($novavar3)ORDER BY criado_em ASC";
...
echo "<script type=\"text/javascript\">document.location.replace('../../tp/direto/pedido_direto_bko_operador.php');
		</script>";
        exit;
?>
</body>
