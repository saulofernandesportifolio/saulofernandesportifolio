...
{
echo "<script>alert('Nenhuma regional selecionada.'); document.location.replace('../../tp/direto/direto_fila_prioriza.php'); </script>\n";
exit;
}
//Regional
$nome = $_SESSION["nome"];
...
if($pedido <> 0 && $data_1 == '0001/01/01')
{
 $sql_consulta="SELECT * FROM ilha_reversao_direto_bko WHERE (fila = 1) and ($novavar2) and ($novavar3) and criado_em BETWEEN '$data_1' and '$data_2 and pedido='$pedido' ORDER BY criado_em ASC";
}
if($pedido <> 0)
{
 $sql_consulta="SELECT * FROM ilha_reversao_direto_bko WHERE (fila = 1) and ($novavar2) and ($novavar3) and pedido='$pedido' ORDER BY criado_em ASC";
}
else
{
$sql_consulta="SELECT * FROM ilha_reversao_direto_bko WHERE (fila = 1) and ($novavar2) and ($novavar3) and criado_em BETWEEN '$data_1' and '$data_2' ORDER BY criado_em ASC";
}
...
echo "<script type=\"text/javascript\">document.location.replace('../../tp/direto/filtro_prioriza_direto.php');
		</script>";
        exit;
