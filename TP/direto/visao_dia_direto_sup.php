...
set_time_limit($tempo);
  ...
  include("../../tp/conexao.php");
  $atv_op2="SELECT * FROM ilha_reversao_direto_bko WHERE data_tramite ='$dt_dia'";
...
?>
           ...
           <p id="p_padrao" align="center">Visão direto Dia: <?php echo $dt_dia2 ?></p>
            <hr width="104%">       
             <?php
			 //tabela por regional - inicio
			 
						include("../../tp/conexao.php");
				$atv_op="SELECT * FROM ilha_reversao_direto_bko WHERE data_tramite ='$dt_dia'";
               ...
               $atv_op="SELECT COUNT(pedido)as pedido,criado_em,usuario,disc_status_tp,regional FROM ilha_reversao_direto_bko WHERE data_tramite = '$dt_dia' GROUP BY regional";
                        $acao_op=mysql_query($atv_op,$conecta);
		...
		while ($dado= mysql_fetch_array($acao_op))
				...
				$atv_op="SELECT * FROM ilha_reversao_direto_bko WHERE data_tramite ='$dt_dia' ";
					...
					$dado= mysql_fetch_array($acao_op);
                        ...
                        $atv_op="SELECT COUNT(pedido)as pedido,criado_em,usuario,disc_status_tp,tipo_erro FROM ilha_reversao_direto_bko WHERE data_tramite = '$dt_dia' GROUP BY tipo_erro";
                        $acao_op=mysql_query($atv_op,$conecta);
			  ...
			  //tabela por motivo - inicio
				...
				$atv_op="SELECT * FROM ilha_reversao_direto_bko WHERE data_tramite ='$dt_dia' ";
					...
					$dado= mysql_fetch_array($acao_op);
                        ...
                        $atv_op="SELECT COUNT(pedido)as pedido,criado_em,usuario,disc_status_tp,descricao_erro FROM ilha_reversao_direto_bko WHERE data_tramite = '$dt_dia' GROUP BY descricao_erro";
                        $acao_op=mysql_query($atv_op,$conecta);
						   //echo  $atv_op;      
		...
		while ($dado= mysql_fetch_array($acao_op))
			...
			//tabela por operador - inicio
					...
					$dado= mysql_fetch_array($acao_op);
                     ...
                     $atv_op="SELECT COUNT(pedido)as pedido ,regional,usuario,disc_status_tp ,criado_em ,tramite ,nome2,data_tramite,turno FROM ilha_reversao_direto_bko WHERE data_tramite = '$dt_dia' and tramite = 'Tratando' or data_tramite= '$dt_dia' and tramite = 'Tratado' or data_tramite = '$dt_dia' and tramite = 'Aguardando' GROUP BY 
