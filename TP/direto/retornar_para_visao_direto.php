...
{
echo "<script>alert('Nenhum regional selecionado.'); document.location.replace('filtro_retornar_para_visao_direto.php'); </script>\n";
exit;
}
if (empty($_POST["tramite"]))
{
echo "<script>alert('Nenhuma tramite selecionada.'); document.location.replace('filtro_retornar_para_visao_direto.php'); </script>\n";
exit;
}
if (empty($_POST["turno2"]))
{
echo "<script>alert('Nenhuma turno selecionada.'); document.location.replace('filtro_retornar_para_visao_direto.php'); </script>\n";
exit;
}	
...
if($data_1 <> ''){
$sql_consulta="SELECT * FROM ilha_reversao_direto_bko WHERE ($regional) and ($tramite) and ($turno) and ($disc_status_tp) and criado_em BETWEEN '$data_1' and '$data_2' ORDER BY criado_em";
$sql=$sql_consulta; 
...
if($numero_pedido <> 0){
$sql_consulta="SELECT * FROM ilha_reversao_direto_bko WHERE pedido='$numero_pedido' ORDER BY criado_em";
$sql=$sql_consulta; 
$acao = mysql_query($sql) or die (mysql_error());
$num = mysql_num_rows($acao);
}
...
if($operador <> '0'){
$sql_consulta="SELECT * FROM ilha_reversao_direto_bko WHERE ($regional) and ($tramite) and ($turno) and ($disc_status_tp) and criado_em BETWEEN '$data_1' and '$data_2' and usuario = '$operador' ORDER BY criado_em";
$sql=$sql_consulta; 
$acao = mysql_query($sql) or die (mysql_error());
$num = mysql_num_rows($acao);
}
...
if($num <= 0)
{
echo "<script>alert('Nenhum pedido com este status.'); document.location.replace('filtro_retornar_para_visao_direto.php'); </script>\n";
exit;
}
//echo $sql;
?>
    ...
    <form name="form1" method="post" action="retornar_para_visao_direto2.php">
...
//Pesquisa e retorna os campos declarado nas variáveis.
        ...
        <td> </td>
        <td>
          <input type="button" name="Submit2" value="Voltar" onClick="window.location='filtro_retornar_para_visao_direto.php'"/><input type="submit" name="Submit" value="Avançar" />
          </td>
          </tr>
