
<?php   
@session_start();

include '../../tp/funcoes.php';

?>
<?php
/**
 * @author Saulo de Assis
 * @copyright 2014
 */
 date_default_timezone_set('Brazil/East');
 
 
if (empty($_POST['tipo_atividade'])){
    
 echo "<script>
                alert('Por gentileza selecionar um tipo valido !');
                history.back();
         </script>";    
    
} 
 
 
 
 

function arrumaString($string) {
 return preg_replace( '/[`^~\'"]/', null, iconv( 'UTF-8', 'ASCII//TRANSLIT',$string)); 
}


function arrumadata($string) {
    if($string == ''){
    $data= substr($string,6,4)."".substr($string,7,2)."".substr($string,0,2);   
        
    }else{
        
    $data= substr($string,6,4)."/".substr($string,7,2)."/".substr($string,0,2);   
    }

 return $data;
}


function arrumadatahora($string2) {
    
    if($string2 == ''){
    $data2=  substr($string2,6,4)."".substr($string2,7,2)."".substr($string2,0,2)." ".substr($string2,10,9);
       }else{
        
      $data2= substr($string2,6,4)."/".substr($string2,7,2)."/".substr($string2,0,2)." ".substr($string2,10,9);
        
       }
return $data2;
}
  $calcula_data = date("d/m/Y");
  
  $data_tratamento=date("Y/m/d");
  $hora_tratamento=date("H:i:s");

$login = $_SESSION["login"];

include '../conexao.php';
 $sql = "SELECT id,login,nome FROM tp.usuarios WHERE login = '".$login."';";
 $consulta = mysql_fetch_assoc(mysql_query($sql)) or die(mysql_error().$sql." erro #SQL_1");
 $id = $consulta['id'];
 $nome1= $consulta['nome'];
 


 
    $tabela = array(
    'n_atividade'=> $_POST['n_atividade'], 
    'regional'=> $_POST['regional'], 
    'data_do_recebimento'=> $_POST['data_do_recebimento'],
    'hora_do_recebimento'=> $_POST['hora_do_recebimento'],
    'remetente'=> $_POST['remetente'], 
    'tipo_atividade'=> $_POST['tipo_atividade'], 
    'pedido'=> $_POST['pedido'], 
    'qtd_linhas'=> $_POST['qtd_linhas'], 
    'criado_em'=> $_POST['criado_em'], 
    'revisao'=> $_POST['revisao'], 
    'adabas'=> $_POST['adabas'], 
    'cnpj'=> $_POST['cnpj'], 
    'cliente'=> arrumaString($_POST['cliente']), 
    'inicio_da_tratativa'=>$_POST['inicio_da_tratativa'], 
    'analista_contestacao'=> $id, 
    'login'=> $id, 
    'ofensor'=> $_POST['tp_ofensor_input'], 
    'data_retorno'=> $_POST['data_retorno'], 
    'tipo2'=> $_POST['motivos_erro_input'], 
    'tipo_apurado'=> $_POST['dc_erro'], 
    'observacoes_colaborador'=>  $obs = $data_cadastro_obs=date("d/m/Y H:i:s")." : ".trim(arrumaString($_POST['parecer']))." "."-"." ".$nome1, 
    'retorno_do_email'=> $retorno = $data_cadastro_retorno=date("d/m/Y H:i:s")." : ".trim(arrumaString($_POST['email']))." "."-"." ".$nome1, 
    'tipo_contestado_FDV'=> $fdv= $data_cadastro_email=date("d/m/Y H:i:s")." : ".trim(arrumaString($_POST['item_fdv']))." "."-"." ".$nome1,
    'analista'=> $_POST['operador_input'], 
    'usuario_att'=> null, 
    'dt_atualizacao'=> null, 
    'dt_retorno2'=>null,
    'cotacao'=> $_POST['cotacao'],
    'data_tratamento'=> $data_tratamento=date("Y/m/d"),
    'hora_tratamento'=>$hora_tratamento=date("H:i:s")
    );
    
$sql = "SELECT 
		CASE WHEN MAX(qtd_contestacoes) IS NULL 
		     THEN 0
		     ELSE MAX(qtd_contestacoes)
		END +1 as qtd_cont
	    FROM base_contestacoes_atividades
	    WHERE n_pedido='".$tabela['pedido']."' AND revisao=".$tabela['revisao']." OR n_atividade='".$tabela['n_atividade']."' AND revisao=".$tabela['revisao']." ;";
 $consulta = mysql_fetch_assoc(mysql_query($sql)) or die(mysql_error().$sql." erro #SQL_2");
 $tabela['qtd_contestacoes'] = $consulta['qtd_cont'];


$data =$data_do_recebimento;
$data_exp_v1 = explode ('/',$data);
$dia = $data_exp_v1[0];
		switch (date('w',mktime(0,0,0,$data_exp_v1[1],$dia,$data_exp_v1[2]))) {
				case 6:
				$teste = 'sabado';
				break;
				default:
			echo	$teste = 'ok';
				break;
		}
							   
$data_modificada_dma = explode('/', $data_do_recebimento);
$data_do_recebimento = $data_modificada_dma[0].'/'.$data_modificada_dma[1].'/'.$data_modificada_dma[2];
$teste1 =calcula_data_sla($data_do_recebimento,$calcula_data);


if($teste == 'sabado'){
	$hora1 = diminui_hora($hora_do_recebimento ,'12:00');
	}else $hora1 = diminui_hora($hora_do_recebimento ,'18:00');
//echo '<BR> hora um =' . $hora1;
$hora_atual = date ('H:i');
$hora2 = diminui_hora('09:00',$hora_atual);
//echo $hora2 . '<br>';
if($hora2 < '00:01'){
	$teste = explode (':' , $hora2);
	$teste2 = $teste[1] * -1;
    $hora2 = '00:' . $teste2;
	}
//echo '<BR> hora um =' . $hora1;
$total_um = soma_hora($hora1,$hora2);
//echo '<BR> total um =' . $total_um;
$total = soma_hora($total_um,$teste1);
$data_modificada_dma = explode('/', $data_do_recebimento);
$data_do_recebimento1 = $data_modificada_dma[2].'-'.$data_modificada_dma[1].'-'.$data_modificada_dma[0];
//echo '<BR> total um =' . $total;
if ($data_do_recebimento == $calcula_data){
	$total = diminui_hora($hora_do_recebimento,$hora_atual);
}



 
 $sql = "insert into `tp`.`base_contestacoes_atividades` 
	(`regional`, 
    `data_do_recebimento`, 
    `hora_do_recebimento`,
    `remetente`, 
	`n_atividade`, 
	`tipo`, 
	`n_pedido`, 
	`qtd_linhas`, 
	`criado_em`, 
	`revisao`, 
	`adabas`, 
	`cnpj`, 
	`cliente`, 
	`inicio_da_tratativa`, 
	`analista_contestacao`, 
	`login`, 
	`ofensor`, 
	`data_retorno`, 
	`tmt`, 
	`tipo2`, 
	`tipo_apurado`, 
	`observacoes_colaborador`, 
	`retorno_do_email`, 
	`tipo_contestado_FDV`, 
	`analista`,
    `qtd_contestacoes`, 
	`usuario_att`, 
	`dt_atualizacao`, 
	`dt_retorno2`,
    `cotacao`,
    `data_tratamento`,
    `hora_tratamento`
	)
	values
	('".
    $tabela['regional']."', '". 
	arrumadata($tabela['data_do_recebimento'])."', '". 
    $tabela['hora_do_recebimento']."', '". 
	$tabela['remetente']."', '".
    $tabela['n_atividade']."', '". 
	$tabela['tipo_atividade']."', '". 
	$tabela['pedido']."', '". 
	$tabela['qtd_linhas']."', '". 
	arrumadata($tabela['criado_em'])."', '".
	$tabela['revisao']."', '".
	$tabela['adabas']."', '".
	$tabela['cnpj']."', '". 
	$tabela['cliente']."', '". 
	arrumadata($tabela['inicio_da_tratativa'])."', '". 
	$tabela['analista_contestacao']."', '". 
	$tabela['login']."', '". 
	$tabela['ofensor']."', '".
	arrumadatahora($tabela['data_retorno'])."', '". 
	$total."', '". 
	$tabela['tipo2']."', '". 
	$tabela['tipo_apurado']."', '". 
	$tabela['observacoes_colaborador']."', '". 
	$tabela['retorno_do_email']."', '". 
	$tabela['tipo_contestado_FDV']."', '". 
	$tabela['analista']."', '". 
    $tabela['qtd_contestacoes']."', '". 
	$tabela['usuario_att']."', '". 
	arrumadatahora($tabela['dt_atualizacao'])."', '". 
	arrumadata($tabela['dt_retorno2'])."', '".  
    $tabela['cotacao']."', '".
    $tabela['data_tratamento']."', '".  
    $tabela['hora_tratamento']."')";
    
    
   
    
 mysql_query($sql) or die(mysql_error().$sql." erro #SQL_2");
 if(mysql_affected_rows() === 1){
    
    die("<script>
                alert('Contestacao inserida com sucesso para a atividade ".$tabela['pedido']."');
                window.location.assign('../../tp/contestacoes/pesquisa_contestacoes_atv.php');
         </script>");
         
 }else{
    
    die("<script>
                alert('Nao foi possivel inserir contestacao, favor verificar os dados inseridos.');
                window.location.assign('../../tp/contestacoes/pesquisa_contestacoes_atv.php');
         </script>");
 }
?>