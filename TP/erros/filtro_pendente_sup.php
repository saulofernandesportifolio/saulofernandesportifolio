...
if (empty($_POST["tipo"]))
{
echo "<script>alert('Nenhum erro selecionado.'); document.location.replace('pendente_sup.php'); </script>\n";
exit;
}
if (empty($_POST["regional"]))
{
echo "<script>alert('Nenhuma regional selecionada.'); document.location.replace('pendente_sup.php'); </script>\n";
exit;
}
if (empty($_POST["carteira"]))
{
echo "<script>alert('Nenhuma carteira selecionada.'); document.location.replace('pendente_sup.php'); </script>\n";
exit;
}
...
if ($contador == 2){
	$filtro_carteira = "and ((vpe='Nao') or (vpe='sim'))";	
	}
if($carteira[0] == "vpe" and $contador != 2){
	$filtro_carteira = "and (vpe='sim')";
	}

if($carteira[0] == "vpg" and $contador != 2){
	$filtro_carteira = "and (vpe='Nao')";
	}
...
$sql_consulta="select * from base_erros where (tramite='Tratando' or tramite='Aguardando') and ($novavar2) and ($novavar) $filtro_carteira order by criado_em";
...
echo "<script type=\"text/javascript\">document.location.replace('../../tp/erros/pedidos_pendentes_sup.php');</script>";
exit;
