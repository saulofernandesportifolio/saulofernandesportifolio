...
{
echo "<script>alert('Nenhuma regional selecionada.'); document.location.replace('../../tp/erros/erros_fila_prioriza.php'); </script>\n";
exit;
}
if (empty($_POST["tipos"]))
{
echo "<script>alert('Nenhum erro selecionado.'); document.location.replace('../../tp/erros/erros_fila_prioriza.php'); </script>\n";
exit;
}
if (empty($_POST["carteira"]))
{
echo "<script>alert('Nenhuma carteira selecionada.'); document.location.replace('../../tp/erros/erros_fila_prioriza.php'); </script>\n";
exit;
}
...
$revi = explode(",", $revisao);
$revi2 = array_slice($revi,2,55);
$novavar4 = implode("", $revi2);



 $sql_consulta="SELECT * from base_erros WHERE (fila = 1) and ($novavar2) and ($novavar3)and ($novavar4)ORDER BY criado_em ASC";
...
echo "<script type=\"text/javascript\">document.location.replace('../../tp/erros/filtro_prioriza_erros.php');
		</script>";
        exit;
