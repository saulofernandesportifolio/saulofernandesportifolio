...
if (empty($_POST["tipo"]))
{
echo "<script>alert('Nenhum erro selecionado.'); document.location.replace('pendente_operador.php'); </script>\n";
exit;
}
if (empty($_POST["regional"]))
{
echo "<script>alert('Nenhuma regional selecionada.'); document.location.replace('pendente_operador.php'); </script>\n";
exit;
}
if (empty($_POST["carteira"]))
{
echo "<script>alert('Nenhuma carteira selecionada.'); document.location.replace('pendente_operador.php'); </script>\n";
exit;
}
...
$sql_consulta="select * from base_erros where (fila=2) and (usuario='$login') and ($novavar2) and ($novavar) and ($filtro_carteira) order by criado_em";
...
$acao_verifica1 = mysql_query($sql_verifca) or die (mysql_error());



echo "<script type=\"text/javascript\">document.location.replace('../../tp/erros/pedidos_pendentes_operador.php');</script>";
exit;
