...
$sql_atv = "SELECT a.id_cotacao,
                   ...
                   a.MIGRACAO_TROCA,
                   a.informacoes    
           FROM cip_nv.tbl_cotacao a,cip_nv.tbl_analise b 
...
<form name="form1" method="post" action="principal.php?&t=controles/sql.distribuir_cotacao_servico1.php">
  ...
  <th colspan="6"><h2>Atualizar Servi&ccedil;o</h2></th> 
    ...
    <tr> 
      <th>Cotacao Principal</th>
      <th>Regional</th>
      <th>Uf</th>
      <th>Criado em</th>
   	  <th>Status Vivocorp</th>
	    <th>Tipo</th>	  
	    </tr>
	    <tr> 
      ...
      <td><?php echo "$uf"?></td>
      <td><?php echo "$criado_em2"?></td>
      <td><?php echo "$status_vivocorp"?></td>
	    <td><select name="tipo" id="tipo" onblur="ValidaEntrada(this,'combo');" class="txt2comboboxpadrao bradius" >
 ...
 ?> </select></td>
     ...
     <tr>
      ...
      <td>
        <input name="informacoes" type="text" id="informacoes" size="4" maxlength="255" value="<?php if($linha['informacoes'] == 0){ echo $linha['informacoes']=""; }else{ echo $linha['informacoes']; } ?> "/>
       </td>   
    </tr>
       <tr>
      <td  colspan="11">Comentários:</td>
	</tr>
    <tr>
     <td colspan="11"><?php echo $descricao; ?></td>
     </tr>
    <?php
  	}
