...
$sql2="CALL cip_nv.visao_pesquisa_diretoria_pendente";


$acao2 = mysql_query($sql2,$conecta) or die (mysql_error());
$num_ = mysql_num_rows($acao2);

if( $num_ == 0){
    
echo "<script>
            alert('Sem pendencias na base.');
...
while($linha_atv2 = mysql_fetch_assoc($acao2))
{
  ...
  $id_diretoria               = $linha_atv2["id"];
  $cotacao_atividade          = $linha_atv2["cotacao_atividade"];
  $n_da_cotacao               = $linha_atv2["n_da_cotacao"];
  $revisao2                   = $linha_atv2["revisao"];
  $regional                   = $linha_atv2["regional"];
  $uf                         = $linha_atv2["uf"];
  $cliente                    = $linha_atv2["cliente"];
  $criado_em                  = arrumadatahora($linha_atv2["criado_em"]);
  $acao                       = $linha_atv2["acao"];
  $segmento                   = $linha_atv2["segmento"];
  $data_do_recebimento        = arrumadata($linha_atv2["data_de_recebimento"]);
  $hora_do_recebimento        = $linha_atv2["hora_de_recebimento"];
  $total_linhas               = $linha_atv2["total_linhas"];
  $solicitante_vivo_accenture = $linha_atv2["solicitante_vivo_accenture"];
  $solicitante                = $linha_atv2["remetente"];
  $tmt                        = $linha_atv2["tmt"];
  $disc_statusdiretoria       = $linha_atv2["disc_statusdiretoria"];
  $operador_diretoria         = $linha_atv2["operador_diretoria"];
  $nome_vivo_accenture        = $linha_atv2["nome_vivo_accenture"];
  $nome_solicitante           = $linha_atv2["nome_solicitante"];
  $operador_diretoria         = $linha_atv2["operador_diretoria"];
...
$formulaor.=$cotacao_atividade." OR ";
...
?>
    <tr>
     ...
     <td class="tdconteudo"><?php echo "<font size='1' color='$cor' face='Arial'>$total_linhas</font>" ?></td>
...
<?php if($num_ > 0){ ?>
<p>Cotenação para pesquisa complementares: <?php echo $formula; ?></p>
<?php } ?> 
