...
ini_set ( 'mysql.connect_timeout' ,  '320' ); 
...
// fim - Atualiza o tipo das atividades na tabela tratamento 
  ...
  $sql_valida = "DELETE FROM tbl_cotacao_vpg 
                 WHERE n_da_cotacao = cotacao_principal and tipo NOT IN ('Analise documentacao') ";
...
/*
$query2="UPDATE tbl_cotacao vpg a,carteira b 
SET a.carteira=b.carteira
WHERE a.cpf_cnpj=b.cpf_cnpj ";
...
$result_cnpj  = mysql_query($update_cnpj ) or die (mysql_error());
 ...
 while($linha_cnpj= mysql_fetch_assoc($result_cnpj)){
    ...
    $acao_valida = mysql_query($sql_valida) or die (mysql_error());
     ...
     /* $update_raiz ="SELECT a.raiz,
       ...
       $acao_atualiza2 = mysql_query($sql_atualiza2) or die (mysql_error()); 
        ...
        $sql_atualiza4 ="SELECT * FROM tbl_cotacao_vpg 
                         WHERE nome_do_gestor LIKE '%Accenture - %' ";

       $acao_atualiza4 = mysql_query($sql_atualiza4) or die (mysql_error()); 
       $numvpe=mysql_num_rows($acao_atualiza4);

       $sql_atualiza3 ="DELETE FROM tbl_cotacao_vpg 
                         WHERE nome_do_gestor LIKE '%Accenture - %' ";
   ...
   $sql_atualiza2 ="SELECT * FROM tbl_cotacao_vpg 
                WHERE n_da_cotacao <> cotacao_principal 
                ...
                and status IN ('Pendente') GROUP BY n_da_cotacao ";    
/*$sql_atualiza2 ="SELECT
b.id_cotacao,
b.numero_da_atividade,
b.novo,
b.incio_planejado,
...
b.numero_do_pedido,
b.status_do_pedido,
b.sub_status_do_pedido,
b.revisao,
b.data_de_criacao_do_pedido,
b.data_da_ultima_atualizacao_do_pedido,
b.n_da_cotacao,
b.status_da_cotacao,
...
b.criado_por,
b.termino_efetivo,
b.regional_atribuida,
b.alta,
b.loja,
b.portabilidade,
b.troca,
b.transferencia_de_titularidade,
b.itens_especiais,
b.responsavel,
b.resultado_analise_credito,
b.descricao,
b.nome_do_responsavel,
b.sobrenome_do_responsavel,
b.cpf_cnpj,
b.raiz,
b.parecer_serasa,
b.parecer_interno,
b.endereco_de_entrega,
b.uf,
b.nome_do_gestor,
b.adabas_do_gn,
b.renegociacao_massiva,
b.analise_na_cotacao,
b.posicao_solicitante_da_SS,
b.nivel_alcada,
b.cotacao_pai,
b.cotacao_principal,
b.os_legado,
b.dt_inclusao_bd_cip,
b.dt_inclusao_bd_cip2,
b.carregado_por_cip,
b.total_linhas_cip,
b.ALTAS,
b.PORTABILIDADE2,
...
b.TIPO_SERVICO,
b.carteira
FROM tbl_cotacao_vpg b 
INNER JOIN tbl_cotacao d ON  b.cotacao_principal=d.cotacao_principal 
WHERE b.n_da_cotacao <> b.cotacao_principal  
                and b.tipo IN ('Analise de input','Ilha de input','Correcao input')
                and b.status_da_cotacao IN ('Enviado ilha de input') 
                and b.substatus_da_cotacao IN ('Input','Analise de input','Correcao input')
                and b.status IN ('Pendente') 
   
GROUP BY b.n_da_cotacao ";*/


    $acao_atualiza2 = mysql_query($sql_atualiza2) or die (mysql_error());
    while($linha_atualiza2 = mysql_fetch_assoc($acao_atualiza2)){
    
        $id_cotacao                           = $linha_atualiza2["id_cotacao"];
        $numero_da_atividade                  = $linha_atualiza2["numero_da_atividade"]; 
        $novo                                 = $linha_atualiza2["novo"];
        $incio_planejado                      = $linha_atualiza2["incio_planejado"];
        $comentarios                          = $linha_atualiza2["comentarios"];
        $tipo                                 = $linha_atualiza2["tipo"];
        $cliente                              = $linha_atualiza2["cliente"];
        $numero_do_pedido                     = $linha_atualiza2["numero_do_pedido"];
        $status_do_pedido                     = $linha_atualiza2["status_do_pedido"];
        $sub_status_do_pedido                 = $linha_atualiza2["sub_status_do_pedido"]; 
        $revisao                              = $linha_atualiza2["revisao"]; 
        $data_de_criacao_do_pedido            = $linha_atualiza2["data_de_criacao_do_pedido"];
        $data_da_ultima_atualizacao_do_pedido = $linha_atualiza2["data_da_ultima_atualizacao_do_pedido"];
        $n_da_cotacao                         = $linha_atualiza2["n_da_cotacao"]; 
        $status_da_cotacao                    = $linha_atualiza2["status_da_cotacao"];
        $substatus_da_cotacao                 = $linha_atualiza2["substatus_da_cotacao"];
        $acao                                 = $linha_atualiza2["acao"];
        $motivo_da_acao                       = $linha_atualiza2["motivo_da_acao"];
        $sub_motivo                           = $linha_atualiza2["sub_motivo"];
        $organizacao_SS                       = $linha_atualiza2["organizacao_SS"];
        $tipo_de_SS                           = $linha_atualiza2["tipo_de_SS"];
        $subtipo_da_SS                        = $linha_atualiza2["subtipo_da_SS"];
        $n_da_SS                              = $linha_atualiza2["n_da_SS"];
        $status_da_SS                         = $linha_atualiza2["status_da_SS"];   
        $substatus_da_SS                      = $linha_atualiza2["substatus_da_SS"];
        $vencimento                           = $linha_atualiza2["vencimento"];
        $prioridade                           = $linha_atualiza2["prioridade"];
        $status                               = $linha_atualiza2["status"];
        $cotacao                              = $linha_atualiza2["cotacao"];
        $faturavel                            = $linha_atualiza2["faturavel"];
        $sobrenome                            = $linha_atualiza2["sobrenome"];
        $nome                                 = $linha_atualiza2["nome"];
        $oportunidade                         = $linha_atualiza2["oportunidade"];
        $funcionarios                         = $linha_atualiza2["funcionarios"];
        $alarme                               = $linha_atualiza2["alarme"];
        $criado_em                            = $linha_atualiza2["criado_em"];
        $criado_por                           = $linha_atualiza2["criado_por"];
        $termino_efetivo                      = $linha_atualiza2["termino_efetivo"];
        $regional_atribuida                   = $linha_atualiza2["regional_atribuida"];
        $alta                                 = $linha_atualiza2["alta"];
        $loja                                 = $linha_atualiza2["loja"];
        $portabilidade                        = $linha_atualiza2["portabilidade"];
        $troca                                = $linha_atualiza2["troca"];
        $transferencia_de_titularidade        = $linha_atualiza2["transferencia_de_titularidade"];
        $itens_especiais                      = $linha_atualiza2["itens_especiais"];
        $responsavel                          = $linha_atualiza2["responsavel"];
        $resultado_analise_credito            = $linha_atualiza2["resultado_analise_credito"];
        $descricao                            = $linha_atualiza2["descricao"];
        $nome_do_responsavel                  = $linha_atualiza2["nome_do_responsavel"];
        $sobrenome_do_responsavel             = $linha_atualiza2["sobrenome_do_responsavel"];
        $cpf_cnpj                             = $linha_atualiza2["cpf_cnpj"];
        $raiz                                 = $linha_atualiza2["raiz"];
        $parecer_serasa                       = $linha_atualiza2["parecer_serasa"];
        $parecer_interno                      = $linha_atualiza2["parecer_interno"];
        $endereco_de_entrega                  = $linha_atualiza2["endereco_de_entrega"];
        $uf                                   = $linha_atualiza2["uf"];
        $nome_do_gestor                       = $linha_atualiza2["nome_do_gestor"];
        $adabas_do_gn                         = $linha_atualiza2["adabas_do_gn"];
        $renegociacao_massiva                 = $linha_atualiza2["renegociacao_massiva"];
        $analise_na_cotacao                   = $linha_atualiza2["analise_na_cotacao"];
        $posicao_solicitante_da_SS            = $linha_atualiza2["posicao_solicitante_da_SS"];
        $nivel_alcada                         = $linha_atualiza2["nivel_alcada"];
        $cotacao_pai                          = $linha_atualiza2["cotacao_pai"];
        $cotacao_principal                    = $linha_atualiza2["cotacao_principal"];
        $os_legado                            = $linha_atualiza2["os_legado"];
        $dt_inclusao_bd_cip                   = $linha_atualiza2["dt_inclusao_bd_cip"];
        $dt_inclusao_bd_cip2                  = $linha_atualiza2["dt_inclusao_bd_cip2"];
        $ALTAS                                = $linha_atualiza2["ALTAS"];
        $PORTABILIDADE2                       = $linha_atualiza2["PORTABILIDADE2"];
        $MIGRACAO                             = $linha_atualiza2["MIGRACAO"];
        $TROCAS                               = $linha_atualiza2["TROCAS"];
        $TT                                   = $linha_atualiza2["TT"];
        $BACKUP                               = $linha_atualiza2["BACKUP"];
        $PRE_POS                              = $linha_atualiza2["PRE_POS"];
        $MIGRACAO_TROCA                       = $linha_atualiza2["MIGRACAO_TROCA"];
        $M_2_M                                = $linha_atualiza2["M_2_M"];
        $FIXA                                 = $linha_atualiza2["FIXA"];
        $TIPO_SERVICO                         = $linha_atualiza2["TIPO_SERVICO"];
        $total_linhas_cip                     = $linha_atualiza2["total_linhas_cip"];
        $carregado_por_cip                    = $linha_atualiza2["carregado_por_cip"];
...
//Monta SQL para INSERT complementar no input
...
//Faz consulta em todas as atividades da tabela tbl_atividades
	...
	/*		
 $query1= "SELECT * FROM tbl_cotacao  WHERE TIPO_COTACAO='Principal' ORDER BY id_cotacao DESC LIMIT 1000 "; 
    ...
    while($linha_query1 = mysql_fetch_assoc($acao_cota2))
    {
        ...
        $TIPO_COTACAO         = $linha_query1["TIPO_COTACAO"];
         ...
         $query2="UPDATE tbl_cotacao a,tbl_cotacao b SET 
          ...
          WHERE a.cotacao_principal  = '$cotacao_principal' and a.revisao=b.revisao  AND a.TIPO_COTACAO='Complementar' AND a.SLA_DIAS IS NULL ";
