...
//}
    ...
    $sql_atualiza ="SELECT * FROM tbl_cotacao_vpg 
                    WHERE status_da_cotacao='Enviado ilha de input' 
					and status='Pendente'";
    $acao_atualiza = mysql_query($sql_atualiza) or die (mysql_error());
    while($linha_atualiza = mysql_fetch_assoc($acao_atualiza)){
      ...
      if($status == "Pendente" 
        ...
        if($n_da_cotacao == $cotacao_principal33 ){
			$tipo_cotacao='Principal';
		}elseif($n_da_cotacao <> $cotacao_principal33){
			$tipo_cotacao='Complementar';
		}
     ...
     $sql_inserir ="INSERT INTO tbl_cotacao(numero_da_atividade, 
             ...
             //Monta SQL para INSERT principal
			...
			//Monta SQL para INSERT complementar
