...
function tiraaspasimples($valor){
  ...
  $hr_dia = date("H:i:s");
	...
	$comentario       = tiraaspasimples($_POST["comentario"]);
...
$data_cadastro_comentario = date('d/m/Y');
 ...
 $criado_em2=arrumadatahora($criado_em);
...
//echo $status_do_pedido;
 $query="INSERT INTO bd_erros_pn.base_erros(
                   pedido,
				   comentario,
				   tipo,
				   portabilidade,
	  ...
	  echo"
		...
		document.location.replace('principal.php?&pedido={$pedido}&t=forms/formatualizar_base_erros_linhas.php');
		</script>
 		";
