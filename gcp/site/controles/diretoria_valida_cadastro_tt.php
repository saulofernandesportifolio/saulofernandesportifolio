...
$acao_operador = mysql_query($sql_operador) or die (mysql_error());
  ...
  $hora_de_finalizacao_da_tratativa=$dttrata;
  ...
  $obscorre=tiraaspasimples($_POST['obs_corre']);
...
$query="UPDATE cip_nv.base_diretoria
        SET 
        ...
        disc_status_cip='$disc_status_cip',
        obs_auditoria='$obscorre',
        operador_cota='$usuario_ciop',
        tipo_servicos='$tipo_servicos'
        WHERE id='$id_diretoria' ";
(!mysql_query($query,$conecta)); 


$sql="CALL cip_nv.visao_pesquisa_diretoria_historico("."'{$id_diretoria}'".")";
