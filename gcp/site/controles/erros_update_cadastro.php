...
$data_cadastro = date("Y-m-d");
 ...
 $sql="SELECT * FROM bd_erros_pn.base_erros WHERE id='$id'";
 ...
 if(!empty($_POST["comentario_novo"])){                             
  ...
  foreach ($_POST["comentario_novo"] as $i=>$comentario_novo) {
     ...
     foreach($_POST['motivo'] as $i=>$mt){
         ...
         //update das linhas                        
         $sql_updatelinhas="UPDATE bd_erros_pn.base_erros_linhas_resumo  
                      SET motivo_erro='$mt'                              
      ...
      foreach($_POST['status_erro'] as $i=>$merro){
          ...
          $sql_updatelinhas="UPDATE bd_erros_pn.base_erros_linhas_resumo  
           ...
           if( $merro != 3){
              ...
              if($merro == 4 && $merro == 5 OR $merro == 4 && $merro == 2 ){
                 $disc_status_erro="Chamado TI";
              }if($merro == 5 && $merro == 2){ 
                 $disc_status_erro="Aguardando Comercial";
              }  
              if($merro == 2){ 
                $disc_status_erro="Em Tratativa";
                     ...
                     //update das linhas                        
             $sql_updatelinhas="UPDATE bd_erros_pn.base_erros  
                      SET   status_tp = '$merro',
			    disc_status_tp = '$disc_status_erro'
                     WHERE  id ='$id' ";
             $updatelinhas = mysql_query($sql_updatelinhas,$conecta2) or die (mysql_error());   
...
$hora_tratamento=date('H:i:s');
     ...
     $comentarioteste =trim($comentario_antigo.$pula.$teste);
      ...
      $sql_update = "UPDATE bd_erros_pn.base_erros SET
				        pedido = '$pedido',
  ...
  $update = mysql_query($sql_update,$conecta2) or die (mysql_error());
   ...
   $sql_updatelinhas="UPDATE bd_erros_pn.base_erros_linhas_resumo  
    ...
    $updatelinhas = mysql_query($sql_updatelinhas,$conecta2) or die (mysql_error());
      ...
      $select_r="SELECT DISTINCT * FROM bd_erros_pn.base_erros_linhas_resumo 
     ...
     while ($dado_r= mysql_fetch_array($update_r)){
          ...
          $sql_insert_r = "CALL bd_erros_pn.carrega_base_erros_linhas_resumo_historico("."'{$id_r}'".")";
  ...
  $update = mysql_query($sql_update,$conecta2) or die (mysql_error());   
    ...
    $sql_insert = "CALL bd_erros_pn.carrega_base_erros_historico("."'{$id}'".")";
	...
	echo"
		...
		document.location.replace('principal.php?t=forms/form_fila_cotacao_erros.php');
		</script>
 		";
