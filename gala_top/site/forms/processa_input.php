...
if($id_filtro != '1234'){
$sql = "SELECT * FROM tbl_usuarios WHERE turno LIKE '%$id_filtro%' and perfil = 3 and status=1 ORDER BY nome ASC ";
...
$sql = "SELECT * FROM tbl_usuarios WHERE turno LIKE '%' and  perfil = 3 and status=1 ORDER BY nome ASC ";
$qr = mysql_query($sql) or die(mysql_error());
echo '<option value="0" selected="selected">Selecione</option>';
   while($ln = mysql_fetch_assoc($qr)){
   echo '<option value="'.$ln['idtbl_usuario'].'">'.$ln['nome'].'</option>';
