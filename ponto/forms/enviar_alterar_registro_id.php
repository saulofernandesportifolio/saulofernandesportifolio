...
$sql_valida = "SELECT * FROM registro_ponto WHERE id = $id";

$acao_valida = mysql_query($sql_valida) or die (mysql_error());

while($linha_valida = mysql_fetch_assoc($acao_valida))
{
	$data_entrada_2				= $linha_valida["data_entrada"];
	$hora_entrada_2				= $linha_valida["hora_entrada"];
	$data_saida_2				= $linha_valida["data_saida"];
	$hora_saida_2				= $linha_valida["hora_saida"];
	$inicio_intervalo_2			= $linha_valida["inicio_intervalo"];	
	$fim_intervalo_2			= $linha_valida["fim_intervalo"];
}

if ($data_entrada_2 == "$data_entrada" and $hora_entrada_2 == "$hora_entrada" and $data_saida_2 == "$data_saida" and $hora_saida_2 == "$hora_saida" and $inicio_intervalo_2 == "$inicio_intervalo" and $fim_intervalo_2 == "$fim_intervalo") 
{

echo "<script>alert('Voce nao realizou nenhuma alteracao no ponto'); history.back(); </script>\n";
exit;
...
if ($dia_ponto == 1)
{
							...
							
							{
										...
										$sql_insert = "UPDATE registro_ponto SET
														usuario = '$login'
														,nome = '$nome'
														,mes = '$mes_registro'
														,data_impressao = '$data_impressao'
														,dia_impressao =  '$diasemana[$dia]'
														,data_entrada = '$data_entrada'
														,hora_entrada = '$hora_entrada'
														,data_hora_ent = '$data_hora_ent'
														,data_saida = '$data_saida'
														,hora_saida = '$hora_saida'
														,data_hora_saida = '$data_hora_sai'
														,inicio_intervalo = '$inicio_intervalo'
														,fim_intervalo =  '$fim_intervalo'
														,falta = '$horas_faltas_ponto'
								...
								if ($horas_trabalhadas > $carga_horaria)
							{
									...
									
									{
											...
											$nova_hora = round($nova_hora/60/60);
