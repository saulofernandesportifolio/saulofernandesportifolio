...
if (!isset($valida))
{
	...
	
	{
		...
		$sql_nome = "SELECT nome, hora_limite_ponto FROM usuarios WHERE id = '$usuario'";
		...
		{
			echo "<script>alert('Hora limite para registro de ponto excedida. Solicite autorizacao a supervisao para registrar sua entrada.'); 
		...
		{ 
			...
			echo "<script>alert('Bom dia, ponto registrado com sucesso.'); 
                   document.location.replace('frame.php?t=conteudo.php'); </script>\n";
			exit;
		}
...
if(isset($valida))
{
	echo "<script>alert('Voce ja possui registro de entrada na data de hoje. Em caso de duvidas consulte a supervisao.'); 
    document.location.replace('frame.php?t=conteudo.php'); </script>\n";
	exit;
}		
