...
$linha_op = mysql_fetch_assoc($acao_op);
  ...
  //$sub_status_vivocorp	= $linha_op["substatus"];
	...
	$status_cip              = $linha_op["status_cip_correcao"];
  $disc_status_cip         = $linha_op["disc_status_cip_correcao"];
...
while($linha_op = mysql_fetch_assoc($acao_op))
{
	$id_cotacao			= $linha_op["id_cotacao"];
	$cotacao_principal	= $linha_op["n_da_cotacao"];
	$disc_status_cip_correcao   = $linha_op["disc_status_cip_correcao"];
...
<tbody>
  ...
  <td class="trcabecalho2 bradius">
  <?php echo "$disc_status_cip_correcao"?></td>
</tr>
 ...
 <?php
  ...
  <br/>
 <input type="button" name="Submit2" value="Voltar" onclick="window.location='../gala/principal.php?&t=forms/form_fila_cotacao_correcao.php'"/>
 </div>
 </div>
</body>
</html>
