             ...
             $cb.=$legenda[$idx].",";
              ...
              if($legenda[$idx] == 'criado_em')
              {
               ...
               $vlr=arrumadatainicio($vlr);
                ...
                if($legenda[$idx] == 'data_vencimento' )
              {
...
// fim - Atualiza o tipo das atividades na tabela tratamento       
 ...
 $sql1="SELECT id_cotacao  FROM tbl_cotacao_filha GROUP BY cotacao_principal";   
      ...
      $sql2="SELECT a.id_cotacao  FROM tbl_correcao a INNER JOIN tbl_cotacao_filha b 
            ON a.id_cotacao=b.id_cotacao 
            WHERE a.idtbl_usuario_correcao ='$idtbl_usuario'  AND dt_carga_bd_cip LIKE '%$data_inclusao_bd2_mes%'
            ";   
...
echo "<font color='#000000' face='arial' size='2'>Cotações filhas  VPG atualizada com sucesso!.<br>";
   if($totalatualizadas2 == 0){
       ...
       if($totalatualizadas2 != 0){
       echo"<br><font color='#000000' face='arial' size='2'>Foram carregados {$totalatualizadas2} cotações filhas no Gala - VPG na Correçao.</font><br>";
       }
