...
$sql_adabas="SELECT count(CPF) as total FROM ativos WHERE CPF='$cpf'";
$acao_adabas = mysql_query($sql_adabas) or die (mysql_error());
$count = mysql_fetch_array($acao_adabas);
 $num=$count['total']; 
  
  
$sql_adabas="SELECT count(CPF) as total FROM ativos WHERE  login='$login'";
$acao_adabas = mysql_query($sql_adabas) or die (mysql_error());
$count = mysql_fetch_array($acao_adabas);
 $num2=$count['total'];           
  ...
  if($num == 0 && $num2 == 0 ){         
						 ...
						 $sql = "INSERT IGNORE INTO ativos (nome_completo,
                                                                    login,
                                                                      CPF
															   )
                                                               VALUES(
