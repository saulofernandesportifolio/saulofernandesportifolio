...
<p>
  <input 
      ...
      value="Novo Usuário" 
      class="sb2 bradius" 
      onClick="window.location='principal.php?id=<?php echo $cripto->codificar($id_usuario_logado)?>&t=views/formusuarios.php'"
  />
</p>
<?php
...
{
  $buscaSolicitacoesUsuariosBySupervisor = mysql_query("SELECT * FROM v_geral_usuario_info WHERE id_perfil > 11
                                                        UNION ALL SELECT * FROM v_geral_usuario_info WHERE id_usuario = $id_usuario_logado");
}
    ...
    while($rowsSolicitacaoUsuario=mysql_fetch_array($buscaSolicitacoesUsuariosBySupervisor)){
      ...
      ?>
       ...
       <tr>
          ...
          <td><a id="td" href="principal.php?&idl=<?php echo $cripto->codificar($id_usuario_logado)?>&id=<?php echo $cripto->codificar($id_usuario)?>&t=views/formusuariosedita.php">
                <?php echo "Editar"; ?>
              </a>&nbsp;
