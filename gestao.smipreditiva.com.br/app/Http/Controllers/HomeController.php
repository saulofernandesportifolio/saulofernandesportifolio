  ...
  public function index()
    {
        ...
        $ano=date('Y');
        $cli1 = '%';
        return view('ocorrencia.grafico',['ocorrencias' => $ocorrencias, 'ocorrencias1' => $ocorrencias1,'ano'=>$ano,
    ...
    public function busca(Request $request)
    {
        ...
        $equi = DB::table('equipamentos')
            ...
            ->select('equipamentos.*', 'ocorrencias.id as idocorrencia', 'ocorrencias.ocorrencia as ocorrencia','equipamentos.cliente as cliente',
                'ocorrencias.velocidade as velocidade', 'ocorrencias.demodulacao as demodulacao', 'ocorrencias.status_geral as status_geral')
        ...
        $cli = DB::table('clientes')->select('id', 'cliente', 'contato')->where('id', 'LIKE', $request->cliente)->get();
        $cli2 = DB::table('clientes')->select('id', 'cliente', 'contato')->where('id', 'NOT LIKE', $request->cliente)->get();
        //count de ocorrencias e feedbacks para montagem dos graficos
        $ocorrencias = DB::table('ocorrencias')->select('mes')->distinct()->get();
    ...
    public function geraPDF($id, Request $request)
    {
        $cli = Cliente::where('id',$id)->get();

        $cli2 = Cliente::where('id',$id)->first();

        $ano = date("Y");

        //valida mês
        $cnt_oc = Ocorrencia::where('idclientes', $id)
            ->where('mes', $request->mes)
            ->where('status_geral','<>','A4')
            ->count();

        if($cnt_oc == 0)
        {
            return back()->with('message','Sem ocorrências no mês selecionado');
        }
            ...
            $equi = DB::table('equipamentos')
                ...
                ->select('equipamentos.*', 'ocorrencias.id as idocorrencia', 'equipamentos.cliente as cliente',
                    'ocorrencias.velocidade as velocidade', 'ocorrencias.demodulacao as demodulacao', 'ocorrencias.status_geral as status_geral')
                ->where('equipamentos.idclientes', 'LIKE', $id)
                ->orderBY('equipamentos.id')
                ->get();

            $ocorrencias = Ocorrencia::where('idclientes', $id)
                ->where('mes', $request->mes)
                ->where('status_geral','<>','A4')
                ->get();
            ...
            return \PDF::loadView('ocorrencia.pdf_geral',['cli'=>$cli, 'equi'=>$equi, 'ocorrencias'=>$ocorrencias, 'mes' => $request->mes, 'ano' => $ano])
                ->setPaper('a4', 'landscape')
                ->download('analise_vibracao-'.$cli2->cliente.'.pdf');
        }
    }
}