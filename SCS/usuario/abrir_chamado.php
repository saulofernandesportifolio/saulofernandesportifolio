...
?>
  <table id="table" class="menu" width="100%">
    ...
    <?php if($sql_chamados["status"]== "TRAMITANDO" || $sql_chamados["status"]== "DEVOLVIDO"){?>
    ...
    <label id="campo_encaminhar" style="display:none">
        ...
        <?php 
		$total_usuarios = mysql_query("SELECT * FROM tbl_usuarios");
				...
				if($sql_chamados["login"] != $usuario["login"]){	
				   ...
				   ">".$usuario["nome"]."</option>";
					...
					$usuario["login"]." id=".$usuario["login"].
					" name=".$usuario["login"].">".$usuario["nome"]."</option>";
					}
				}
				?>
            </select>
