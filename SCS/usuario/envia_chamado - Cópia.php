...
//Cria chamado
 $sql_chamado = "INSERT INTO `scs`.`tbl_chamados` (`n_chamado`, `solicitacao`, `tipo`, `sistema`, `descricao`, `login`, `arquivo_user`, `arquivo_op`, `dt_solic`, `dt_conclusao`, `dt_devolucao`, `dt_finalizacao`, `status`, `prioridade`) VALUES ('".$n_chamado."', '".$solicitacao."', '".$tipo."','".$sistema."', '"."[".$login."] ".$descricao."', '".$sql_usuario_logado['login']."', 'AGUARDANDO','".htmlentities($file_name)."',NULL,'".date("Y-m-d")."',NULL,NULL,NULL, 'TRAMITANDO', '2')";
...
mysql_query($sql_chamado) or die(mysql_error());
		...
		echo "<script>
		     alert('$erro \\nChamado Numero $n_chamado foi criado com sucesso!'); 
	          document.location.replace('menu_1.php?ide={$ide}&n_chamado={$n_chamado}&m=1');</script>";
	     exit;		
?>