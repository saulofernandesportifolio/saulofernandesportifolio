...
include "../bd.php";
$sql_tramitando = mysql_query("SELECT * FROM tbl_chamados WHERE (status <> 'CONCLUIDO') ORDER BY prioridade, status ASC");
...
//Controla se os dados que serão exibidos estão de acordo com o solicitado (de 10 em 10)	
		if($i >= ($limite * 10) && $i < mysql_num_rows($sql_tramitando)){
			...
			echo '<tr>
				...
				<td align="center" width="18%"><a ',$classe,'>',$tipo,'</td>
...
//Constrói o rodapé da tabela
		echo "<tr>
				...
				<td colspan=\"7\"><font><center>Exibindo do <strong>".(($limite*10)+1)."</strong> até o <strong>".(($limite*10)+$cont)."</strong> de um total de <strong>"
