...
function HabCampos2() 
{
	  if (document.getElementById('atender').checked) 
	  {
		document.getElementById('campo_encaminhar').style.display = "none";
		document.getElementById('campo_atender').style.display = "";
		document.getElementById('selectfield').focus();
	  }
	   else 
	  {
		
		document.getElementById('campo_atender').style.display = "none";
	  }
	  if (document.getElementById('encaminhar').checked) 
	  {
		document.getElementById('campo_encaminhar').style.display = "";
		document.getElementById('campo_atender').style.display = "none";
		document.getElementById('selectfield').focus();
	  }
	   else 
    ...
    <script type="text/javascript">
		...
		d.add(0,-1,'SCS - M&oacute;dulo Coordenador');
		  d.add(1,0,'Chamados');
			...
			d.add(3,1,'Meus Chamados',
			'menu_1.php?ide=<?php echo "$idusuario"?>&m=12','','_self');
            d.add(4,1,'Aguardando Tratamento',
			'menu_1.php?ide=<?php echo "$idusuario"?>&m=2','','_self');
		    d.add(5,1,'Chamados conclu&iacute;dos',
			'menu_1.php?ide=<?php echo "$idusuario"?>&m=3','','_self');
		    d.add(6,0,'Gerenciar Usu&aacute;rios');
            d.add(7,6,'Criar Usu&aacute;rio',
			'menu_1.php?ide=<?php echo "$idusuario"?>&m=5','','_self');
            d.add(8,6,'Exibir Usu&aacute;rios',
			'menu_1.php?ide=<?php echo "$idusuario"?>&m=6','','_self');
			d.add(9,0,'Base Chamados');
		    d.add(10,9,'Exportar Base',
			'menu_1.php?ide=<?php echo "$idusuario"?>&m=8','','_self');
			d.add(11,0,'Meu Perfil');
            d.add(12,11,'Alterar Senha',
			'menu_1.php?ide=<?php echo "$idusuario"?>&m=9','','_self');
            d.add(13,11,'Logout',
			'menu_1.php?ide=<?php echo "$idusuario"?>&m=10','','_self');
		d.draw();
		//-->
	</script>
</div>
...
if($perfil == 1){
			switch ($m){
				...
				case 3:
					...
					//valor reservado para tratamento do chamado
			    case 4:
					include "abrir_chamado.php";
					break;
				//Atualização: 19/09/2013 - Lauro Pereira
				case 11:
				...
				case 6:
					include "exibe_usuario.php";
					break;
					
					//valor reservado para abertura de usuario
					case 7:
						include "abrir_usuario.php";
						break;
				case 8:
					include "exporta_base.php";
					break;
				...
				case 10:
					...
					break;
                case 12:
					include "pesquisa_tramitando_usuario.php";
					break;
                case 13:
					include "grafico.php";
					break;
				default:
					echo "<center><img width='80%' height='80%' src='../img/logo_empreza_1.jpg' /></center>";
			}
	}
}
?>
